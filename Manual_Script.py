...
class World(object):
    ...
    def restart(self):
        ...
        blueprint = random.choice(self.world.get_blueprint_library().filter("vehicle.seat.leon"))
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(carla.Location(x=381, y=-2.1, z=0.5), carla.Rotation(yaw=180))
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
        # Set up the sensors.
...
def depth_process_image(image):
    ...
    global i 
    i = i +1
    #depth_img_processed_img.save("controller_output/sensors_output/Depth_Before/Depth_"+str(i)+".png") 
                ...
                try:
                   ...
                   # if in_a_row >= 2:
                    ...
                    #    cv2.imwrite("controller_output/visual_output/curved_lane_img"+str(frame)+".jpg", Curved_Lane)
                        #Curved_Lane.save("controller_output/visual_output/curved_lane_img"+str(frame)+".jpg")

                    #    image_from_array = Image.fromarray(image_rgb)
                    #    rgb_image_array = image_from_array.save("controller_output/sensors_output/Image_"+str(frame)+".jpg") 
                    #cv2.imwrite("controller_output/sensors_output/depth"+str(frame)+".jpg", Curved_Lane)
                    #print("2")
                    #depth_Array = depth_image_array_global.T
                    ...
                    #rotated = ndimage.rotate(in_meters, -90)
                    #rotated =  cv2.flip(rotated, 1)
                    #cv2.imwrite("controller_output/sensors_output/depth_meters"+str(frame)+".jpg", rotated)
                    image_rgb = cv2.cvtColor(rgb_image_array_global,cv2.cv2.COLOR_BGR2RGB)
                    cv2.imwrite("controller_output/sensors_output/camera_"+str(frame)+".jpg", image_rgb)
                    print(2)
